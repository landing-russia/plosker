<section id="testimonials" class="py-12 lg:py-20 bg-slate-900">
  <h2
    class="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-500 sm:text-5xl sm:leading-none lg:text-6xl text-center"
  >
    Отзывы
  </h2>
  <div
    class="container-fluid max-w-screen-2xl mx-auto lg:grid lg:grid-cols-3 py-12"
  >
    <div
      class="px-4 pb-12 lg:pb-0 sm:px-6 lg:flex lg:flex-col lg:pl-0 lg:pr-10 lg:border-r md:border-slate-800"
    >
      <div class="lg:flex-shrink-0">
        <img
          class="h-20 w-auto"
          width="276"
          height="127"
          src="shikardo.png"
          alt="Shikardo"
        />
      </div>
      <blockquote class="mt-6 lg:flex lg:flex-col">
        <div class="relative text-lg font-medium text-slate-100 lg:flex-grow">
          <svg
            class="absolute top-0 left-0 transform -translate-x-3 -translate-y-2 h-8 w-8 text-slate-700"
            fill="currentColor"
            viewBox="0 0 32 32"
            aria-hidden="true"
          >
            <path
              d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z"
            />
          </svg>
          <p class="relative italic font-light">
            Мы&nbsp;высоко ценим совместную работу с&nbsp;2019 года с&nbsp;Вашей компанией и&nbsp;выражаем огромную признательность за&nbsp;плодотворное сотрудничество. Мы&nbsp;искренне благодарим Вас за&nbsp;высокий уровень профессионализма и&nbsp;внимательное отношение.
          </p>
        </div>
        <footer class="mt-8">
          <div class="flex items-start">
            <!-- <div
              class="flex-shrink-0 inline-flex rounded-full border-2 border-white"
            >
              <img
                class="h-12 w-12 rounded-full"
                src="women-testimonials.jpg"
                alt=""
              />
            </div> -->
            <div class="font-bitter italic">
              <div class="text-base font-semibold text-slate-300">
                Богаченко&nbsp;А.&nbsp;В.
              </div>
              <div class="text-sm font-medium text-slate-400">Салон «Шикардо»</div>
            </div>
          </div>
        </footer>
      </blockquote>
    </div>
    <div
      class="px-4 py-12 lg:py-0 border-t-2 border-slate-800 sm:px-6 lg:pr-10 lg:border-t-0 lg:border-l"
    >
      <div class="lg:flex-shrink-0">
        <img
          class="h-20 w-auto"
          width="300"
          height="212"
          src="ultramarin.png"
          alt="Ultramarin"
        />
      </div>
      <blockquote class="mt-6 lg:flex lg:flex-col">
        <div class="relative text-lg font-medium text-slate-100 lg:flex-grow">
          <svg
            class="absolute top-0 left-0 transform -translate-x-3 -translate-y-2 h-8 w-8 text-slate-700"
            fill="currentColor"
            viewBox="0 0 32 32"
          >
            <path
              d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z"
            />
          </svg>
          <p class="relative italic font-light">
            Plosker Groupe ведёт бухгалтерский учёт нашей компании с&nbsp;декабря 2016&nbsp;г. На&nbsp;протяжении этого периода нам оказываются качественные услуги: своевременно составляется и&nbsp;подаётся отчётность во&nbsp;все бюджетные и&nbsp;внебюджетные фонды, проводятся сверки взаиморасчетов с&nbsp;контрагентами, успешно проходят налоговые проверки, предоставляются налоговые консультации высокого уровня.
          </p>
        </div>
        <footer class="mt-8">
          <div class="flex items-start">
            <!-- <div
              class="flex-shrink-0 inline-flex rounded-full border-2 border-white"
            >
              <img
                class="h-12 w-12 rounded-full"
                src="women-testimonials.jpg"
                alt=""
              />
            </div> -->
            <div class="font-bitter italic">
              <div class="text-base font-semibold text-slate-300">
                Кулаков&nbsp;В.&nbsp;Г.
              </div>
              <div class="text-sm font-medium text-slate-400">Отель «Ультрамарин»</div>
            </div>
          </div>
        </footer>
      </blockquote>
    </div>
    <div
      class="px-4 pt-12 lg:pt-0 border-t-2 border-slate-800 sm:px-6 lg:pr-10 lg:border-t-0 lg:border-l"
    >
      <div class="lg:flex-shrink-0">
        <img
          class="h-20 w-auto"
          width="300"
          height="212"
          src="yurdizo.png"
          alt="Yurdizo"
        />
      </div>
      <blockquote class="mt-6 lg:flex lg:flex-col">
        <div class="relative text-lg font-medium text-slate-100 lg:flex-grow">
          <svg
            class="absolute top-0 left-0 transform -translate-x-3 -translate-y-2 h-8 w-8 text-slate-700"
            fill="currentColor"
            viewBox="0 0 32 32"
          >
            <path
              d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z"
            />
          </svg>
          <p class="relative italic font-light">
            Plosker Groupe является надёжным партнёром в&nbsp;области ведения бухгалтерского и&nbsp;налогового учёта. Высокий профессионализм сотрудников обеспечивает максимальную оперативность в&nbsp;решении любых задач, позволяет нам избежать налоговых потерь и&nbsp;вести свой бизнес эффективно.
          </p>
        </div>
        <footer class="mt-8">
          <div class="flex items-start">
            <!-- <div
              class="flex-shrink-0 inline-flex rounded-full border-2 border-white"
            >
              <img
                class="h-12 w-12 rounded-full"
                src="women-testimonials.jpg"
                alt=""
              />
            </div> -->
            <div class="font-bitter italic">
              <div class="text-base font-semibold text-slate-300">
                Ламберски&nbsp;Т.&nbsp;Ю.
              </div>
              <div class="text-sm font-medium text-slate-400">Юридический департамент имущественных и&nbsp;земельных отношений</div>
            </div>
          </div>
        </footer>
      </blockquote>
    </div>
  </div>
</section>
